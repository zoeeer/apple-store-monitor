...
def forward(old_orm, new_orm):
    availabilityhistory = new_orm['availabilityhistory']
    return [
        # Apply value 0 or 1 to the field availabilityhistory.inventory,
        availabilityhistory.update({availabilityhistory.inventory: 1}).where(availabilityhistory.is_available == True),
        availabilityhistory.update({availabilityhistory.inventory: 0}).where(availabilityhistory.is_available == False),
    ]


def migrate_forward(op, old_orm, new_orm):
    op.run_data_migration()
    op.add_not_null(new_orm.availabilityhistory.inventory)


def migrate_backward(op, old_orm, new_orm):
    op.run_data_migration()
    op.drop_not_null(new_orm.availabilityhistory.inventory)
